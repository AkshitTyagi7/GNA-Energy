...
export function Gnai() {
        ...
        <div className="h-full">
            <div className="chatArea pb-10 rounded " style={{ scrollBehavior: 'smooth' }} ref={
                ...
                {messages.map((message, index) => {
                    return <ChatBox key={index} role={message.role} message={message.content} />
                })}
                ...
                <form onSubmit={_handleMessageSubmit}>
                    <div className="flex justify-center h-full  text-center gptInputBox bg-white pl-2 " >
                        ...
                        <div className="pl-2 w-full flex">
                            <input type="text" className="flex-grow h-full border-none outline-0 " id="gptInput" required={true} placeholder="Send a message...." />
                            <button> <Send type="submit" className="h-full mr-2 ml-2" /></button>
  ...
  async  function _handleMessageSubmit(e: any) {
        ...
        const response = await fetch("https://assistant.gna.energy/api/streamChatGpt", {
            method: "post",
            headers: {
        ...
        <div className="messageBox p-6">
            <div className="flex space-x-2">
                <div className="flex-shrink-0">
                    {role === Role.User ? <UserIcon className="h-10 w-10" /> : <AiIcon className="h-10 w-10" />}
                </div>
                <div>
