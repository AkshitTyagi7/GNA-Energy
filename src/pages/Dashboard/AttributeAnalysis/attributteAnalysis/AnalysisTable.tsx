...
import GetEnergyData, {
  ConvertEnergyData,
} from "../../../../components/charts/data/EnergyChargeData";
    ...
    <tr className="h-[49px]">
      ...
      <td className="actualmu ">
        <td className="tablefield">
          {isAdjusted ? record.Adjusted_Mus : record.Actual_Mus}
        </td>
      </td>
      <td className="optimalmu">
        <td className="tablefield">
          {isAdjusted ? record.variable_cost : record.variable_charge}
        </td>
      </td>
      <td>
        <div className="tablefield">
          {isAdjusted ? (
            <Slider
              currentValue={(
                parseFloat(
                  record.Adjusted_Availability.toString()
                    .replace("%", "")
                    .replace(" ", "")
                ) * 100
              ).toFixed(2)}
            ...
            (
              parseFloat(record.Availability.toString().replace("%", "")) * 100
            ).toFixed(2)
          )}
        </div>
      </td>
        ...
        <div className="tablefield">
          {isAdjusted ? (
            <Slider
              currentValue={
                isAdjusted
                  ? record.Adjusted_variable_charge
                  : record.variable_charge
              }
              minValue={0}
              maxValue={20}
              onChange={(v: any) =>
                onAdjustedVariableChange(v, record.Generator)
              }
            />
          ) : (
            record.variable_charge
            ...
            <div>
              ...
              <div className="flex justify-between">
                <div className="gapRow">
                  <button
                    className={`btn rectangle btn-primary btn-small ${
                      !isAdjusted ? "btn-active" : ""
                    }`}
                    onClick={() => {
                      setIsAdjusted(false);
              ...
              <div className="tableArea">
                <>
                  <table className="analysisTable">
                    <thead>
                      <tr>
                        <th onClick={() => {}}>Generator</th>

                        <th
                          onClick={() => {
                            setTableRecords(
                              tableRecords.sort(
                                (a, b) => a.Actual_Mus - b.Actual_Mus
                              )
                            );
                            // setTableRecords([]);
                        ...
                        <th
                          onClick={() => {
                            ...
                            setTableRecords([]);
                            newRecord.sort(
                              (a, b) => a.variable_cost - b.variable_cost
                            );
                        ...
                        <th
                          onClick={() => {
                            setTableRecords(
                              tableRecords.sort(
                                (a, b) =>
                                  parseFloat(a.Availability) -
                                  parseFloat(b.Availability)
                              )
                            );
                          }}
                        >
                        ...
                        <th
                          onClick={() => {
                            setTableRecords(
                              tableRecords.sort(
                                (a, b) => a.variable_charge - b.variable_charge
                              )
                            );
                          }}
                        >
...
<TableRow key={record.Generator} record={record} onChange={onChange} maxOptimalMus={maxOptimalMus} />
          ...
          <div className="item2">
            <div className="flex space-x-2 mb-2 items-center justify-center"></div>
            <Cards />
                    ...
                    <Bar
                      ...
                      data={
                        msedChartIndex === 0
                          ...
                          : msedChartIndex === 1
                          ? PrepareGraphData(
                              ConvertTotalCostData(data.total_cost)
                            )
                          : msedChartIndex === 2
                          ? PrepareGraphData(
                              ConvertEnergyData(data.energy_charge)
                            )
                          : msedChartIndex === 3
                          ? PrepareGraphData(ConvertPpaUtil(data.ppa_util))
                          : { labels: [], datasets: [] }
                      }
                      options={
                        msedChartIndex === 0
                          ? GetChartOptions({
                              textTitle: `Total Procurement (${ENERGY_UNIT})`,
                              yLabelText: ENERGY_UNIT,
                              displayYLabel: true,
                              displayLegend: true,
                          ...
                          : msedChartIndex === 2
                          ? GetChartOptions({
                              textTitle: `Variable Energy Charge (${COST_UNIT})`,
                              yLabelText: COST_UNIT,
                              displayYLabel: true,
                              displayLegend: false,
                              maintainAspectRatio: false,
                            })
              ...
              </div>
              {
                <div className="flex mt-5 text-center center justify-between space-x-0">
                  <div className="col-3 pie grow-0">
                    <PieChart
                      data={ConvertSourceData(data.source)}
                  ...
                  <div className="col-3 grow">
                    <BarChart
                      data={ConvertOwnershipData(data.ownership)}
                      options={GetChartOptions({
                        textTitle: "By Ownership",
                        yLabelText: "MUs",
                        displayYLabel: true,
                        displayLegend: false,
                        isSmallLegend: false,
  ...
  function MahagencoButton({
    ...
    return (
      <button
        className={`btn rectangle btn-primary btn-small ${
          mahagencoChartIndex === index ? "btn-active" : ""
        }`}
        onClick={() => {
  ...
  function MSEDButton({
    ...
    return (
      <button
        className={`btn rectangle btn-primary btn-small ${
          msedChartIndex === index ? "btn-active" : ""
        }`}
        onClick={() => {
    ...
    return (
      <div className="cards">
        ...
        <TotalCard
          ...
          totalText="Total Cost - Cr (Actual)"
        />
        <TotalCard
          totalAmount={data.cost[2]}
          totalText="Total Cost - Cr (Plan)"
        />
      </div>
    );
  }
  ...
  async function fetchTableData() {
    try {
      ...
      const response = await fetch(
        "https://datahub.gna.energy/attribution_analysis_api",
        {
          method: "POST",
      ...
      const newRecords = newdata.data.map((element) => element as TableRecord);
      let maxAdjusted_Mus = newRecords.reduce((prev, current) =>
        prev.Adjusted_Mus > current.Adjusted_Mus ? prev : current
      );
